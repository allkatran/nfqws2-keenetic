...
Проверка доступности легко обманывает. Ниже — то, на чём я обжёгся сам, пока настраивал
пакет (Netcraze NC-1812, Ростелеком, Москва).
...
3. **Минимум три прогона на цель.** Отказы плавающие: у меня одна и та же цель за
   15 минут меняла статус четыре раза. По одному прогону вывод делать нельзя.
4. **Один успешный запрос ≠ работающий сайт.** Страница — это 20+ запросов к разным
   хостам. Проверять надо набором целей, а не одним `GET`.
...
#### Широкие диапазоны в `ipset.list` вредят
...
**70,8 % отказов TLS (34 из 48 прогонов)**, хосты вне этих диапазонов — **0 % (0 из 69)**.
При этом `162.158.0.0/15` не ломался ни разу (9 из 9). Похоже на
[bol-van/zapret#1756](https://github.com/bol-van/zapret/issues/1756): при сбое пересборки
TLS оригинальный пакет уходит раньше фейкового, и вместо обхода получается таймаут
рукопожатия.
